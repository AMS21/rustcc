...
mod tests {
    ...
    use super::*;
    use crate::{diagnostic::DiagnosticId, diagnostic_consumer::IgnoreDiagnosticConsumer};

    #[test]
    fn diagnostic_engine_new() {
        let consume = Box::new(IgnoreDiagnosticConsumer);
        let engine = DiagnosticEngine::new(consume);

        assert_eq!(engine.number_of_warnings, 0);
        assert_eq!(engine.number_of_errors, 0);
        assert_eq!(engine.error_limit, 0);
        assert!(!engine.error_occurred);
        assert!(!engine.fatal_error_occurred);
        assert!(!engine.ignore_all_warnings);
    ...
    #[test]
    fn diagnostic_engine_report_ignored() {
        let consume = Box::new(IgnoreDiagnosticConsumer);
        let mut engine = DiagnosticEngine::new(consume);
    ...
    #[test]
    fn diagnostic_engine_report_warning() {
        let consume = Box::new(IgnoreDiagnosticConsumer);
        let mut engine = DiagnosticEngine::new(consume);
        ...
        assert_eq!(engine.number_of_warnings, 1);
        assert_eq!(engine.number_of_errors, 0);
        assert!(!engine.error_occurred);
        assert!(!engine.fatal_error_occurred);
    ...
    #[test]
    fn diagnostic_engine_report_error() {
        let consume = Box::new(IgnoreDiagnosticConsumer);
        let mut engine = DiagnosticEngine::new(consume);
