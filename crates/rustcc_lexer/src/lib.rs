...
use rustcc_source::{
    source_file::SourceFile, source_location::SourceLocation, source_range::SourceRange,
};
...
impl<'a> Lexer<'a> {
    ...
    pub fn tokenize(&'_ mut self) -> TokenList<'_> {
        ...
        match self.state {
            LexerState::Start => match self.peek_next() {
                ...
                Some('&') => {
                    ...
                    self.state = LexerState::AfterAmpersand;
                    self.consume_character();
                }
                Some('|') => {
                    let location = self.current_location();
                ...
                Some('=') => {
                    ...
                    self.state = LexerState::AfterEquals;
                    self.consume_character();
                }
                Some('!') => {
                    let location = self.current_location();
            ...
            LexerState::AfterAmpersand => match self.peek_next() {
                Some('&') => {
                    let location =
                        SourceRange::new(self.token_begin_location, self.current_location());
                ...
                _ => {
                    self.queued_tokens
                        .push_back(Token::new_ampersand(self.token_begin_location));
            ...
            LexerState::AfterPipe => match self.peek_next() {
                ...
                _ => {
                    self.queued_tokens
                        .push_back(Token::new_pipe(self.token_begin_location));
            ...
            LexerState::AfterBang => match self.peek_next() {
                Some('=') => {
                    ...
                    self.consume_character();
                    self.queued_tokens
                        .push_back(Token::new_bang_equals(location));
            ...
            LexerState::AfterEquals => match self.peek_next() {
                Some('=') => {
                    let location =
                        SourceRange::new(self.token_begin_location, self.current_location());
                    ...
                    self.consume_character();
                    self.queued_tokens
                        .push_back(Token::new_equals_equals(location));
                ...
                _ => {
                    self.queued_tokens
                        .push_back(Token::new_equals(self.token_begin_location));
