...
/// - Strip leading "crates/" segments
/// - Normalize CRLF to LF line endings
fn normalize_output_for_compare(s: &str) -> String {
    // Normalize line endings and path separators
    let mut s = s.replace("\r\n", "\n");
    s = s.replace('\\', "/");

    // Strip the explicit "crates/" segment wherever it appears.
    s = s.replace("crates/", "");

    // Collapse any accidental duplicate path separators
    s = s.replace("//", "/");

    // Normalize Windows program suffix in clap Usage lines
...
fn main() -> ExitCode {
    ...
    for input_path in &input_files {
        ...
        let Ok(output) = command
            ...
            println!("{}", "TEST ERROR".red());
            println!("Failed to execute binary '{executable}'");
        ...
        if update_baseline {
            ...
            fs::write(output_path, &normalized_output_str).expect("Failed to write output file");
            ...
            // Compare the output
            let expected_output_norm = normalize_output_for_compare(&expected_output);
            if normalized_output_str.trim() == expected_output_norm.trim() {
                println!("{}", "PASS".green());
            } else {
                println!("{}\n", "FAIL".red());
